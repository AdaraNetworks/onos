...
public class MQEventHandlerTest extends AbstractEventTest {
    ...
    private ApplicationId appId = new DefaultApplicationId(100, "org.onosproject.rabbitmq");
    ...
    private class TestDeviceService extends DeviceServiceAdapter {
        ...
        public TestDeviceService() {
            Device d1 = new DefaultDevice(ProviderId.NONE, DID1, Device.Type.SWITCH, "TESTMF", "TESTHW", "TESTSW",
                    "TESTSN", new ChassisId());
            Device d2 = new DefaultDevice(ProviderId.NONE, DID2, Device.Type.SWITCH, "TESTMF", "TESTHW", "TESTSW",
                    "TESTSN", new ChassisId());
            devices.put(DID1, d1);
            devices.put(DID2, d2);
    ...
    private class TestPacketContext implements PacketContext {
        ...
        public InboundPacket inPacket() {
            ONOSLLDP lldp = ONOSLLDP.onosLLDP(deviceService.getDevice(DID1).id().toString(), device.chassisId(),
    ...
    private void submitTopologyGraph() {
        ...
        Set<Link> links = of(link("a", 1, "b", 1), link("b", 1, "a", 1), link("b", 2, "c", 1), link("c", 1, "b", 2),
                link("c", 2, "d", 1), link("d", 1, "c", 2), link("d", 2, "a", 2), link("a", 2, "d", 2),
    ...
    private Link createLink() {
        return DefaultLink.builder().providerId(new ProviderId("of", "foo"))
                ...
                .dst(new ConnectPoint(deviceId("of:bar"), portNumber(2))).type(Link.Type.INDIRECT).build();
    }
